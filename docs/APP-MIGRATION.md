...
HeangOS is the destination application. The migration uses a strangler approach:
routes move into HeangOS one working slice at a time while the existing Supabase
projects continue to own their data.

## Test slice

| HeangOS route | Source product | Data source | Session model | Current access |
| --- | --- | --- | --- | --- |
| `/finance` | Money Flow | Money Flow Supabase | Separate browser session | Read-only monthly summary and recent transactions |
| `/learning` | Hengo | Hengo Supabase | Shared HeangOS session | Read-only profile, vocabulary, reviews, activity, and latest plan |
...
Hengo is the primary HeangOS identity. Google login uses Hengo's existing Google
...
HeangOS therefore stores the Money Flow session under the isolated
`heangos-money-flow-auth` browser key. Google users link Money Flow with one OAuth
...
project used by HeangOS Auth. Finance additionally needs:
...
2. Port Money Flow transactions, budgets, savings, analytics, and settings as
   native HeangOS routes, preserving Money Flow RLS and tables.
3. Port Hengo vocabulary, daily study, interview, progress, and supporting
   learning routes, reusing the primary Hengo/HeangOS session.
4. Move shared UI and domain code into HeangOS modules; do not import the old
...
The separate Neon database remains the store for native HeangOS tasks, goals,
and future HeangOS-owned domains. A later data consolidation is a separate
